...
class MicroblogUser(HttpUser):
    ...
    def on_start(self):
        ...
        with self.client.post("/auth/register", data=reg_data, catch_response=True, timeout=20) as res1:
            if res1.status_code != 200:
                res1.failure(f"Failed to register. Response code: {res1.status_code}")

        with self.client.post("/auth/login-api", json=log_data, catch_response=True, timeout=20) as res2:
    ...
    def view_homepage(self):
        with self.client.get("/index", catch_response=True, timeout=20) as resp:
    ...
    def create_post(self):
        ...
        with self.client.post("/index-api", headers=headers, json=form_data, cookies={"session": self.sessionID}, catch_response=True, timeout=20) as resp:
            if resp.status_code != 200:
    ...
    def view_archive(self):
        with self.client.get("/archive-api", cookies={"session": self.sessionID}, catch_response=True, timeout=20) as resp:
            if resp.status_code != 200:
    ...
    def create_and_like_post(self):
        ...
        with self.client.post("/create-like-post-api", headers=headers, json=form_data, cookies={"session": self.sessionID}, catch_response=True, timeout=20) as resp:
            if resp.status_code != 200:
    ...
    def enable_disable_2fa(self):
        ...
        with self.client.post("/auth/manage_authentication_api", headers=headers, json=form_data, cookies={"session": self.sessionID}, catch_response=True, timeout=20) as resp:
            if resp.status_code != 200:
